...
ROUNDS = 5
ON_TIME_MIN = 45
FINE_PER_MIN = 100            # fine paid by each role
# Task baselines
...
MX_FIX_COST = 300
MX_PENALTY  = 1000
MX_PEN_PROB = 0.4             # 40 % penalty risk
# Event cards
EVENT_CARDS = [
...
def build_timeline(idx):
    ...
    if fine:
        for r in ROLES:
            st.session_state.data[r].at[idx, "Cost"] += fine  # each role pays full fine

def record(role: str, idx: int, choice: str):
...
def kpi_strip(role: str):
    col1, col2 = st.columns(2)
    col1.metric("Your Cost", f"${int(st.session_state.data[role]['Cost'].sum()):,}")
    last = latest_time()
    col2.metric("Latest Ground Time", f"{last} min", delta=f"{last - ON_TIME_MIN:+}")
...
with tab_help:
    ...
    st.markdown(
        ...
        "5. Study the timeline board — then repeat."
    )
